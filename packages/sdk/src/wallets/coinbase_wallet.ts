 ...
 * 
 * Browser wallet adapter for Coinbase Wallet.
...
export const COINBASE_WALLET_KIND = "coinbase_wallet";
...
export interface CoinbaseWalletOptions {
  ...
  injected?: {
    ...
    address?: string; // Default: "0xabcdefabcdefabcdefabcdefabcdefabcdefabcd"
...
export class CoinbaseWalletAdapter implements WalletAdapter {
  private injected?: CoinbaseWalletOptions["injected"];
  private address?: string;
  private connected = false;
  public readonly kind = COINBASE_WALLET_KIND;
  ...
  async connect(): Promise<void> {
    ...
    if (!this.injected) {
      ...
      // In browser, try to access window.coinbaseWalletExtension (but this is for future real implementation)
    ...
    this.address = this.injected.address || "0xabcdefabcdefabcdefabcdefabcdefabcdefabcd";
    this.connected = true;
  }
  ...
  async signMessage(message: Uint8Array): Promise<Uint8Array> {
    ...
    // Fill rest with deterministic pattern (different pattern than MetaMask for differentiation)
    for (let i = 32; i < 65; i++) {
      sig[i] = (hashArray[i % hashArray.length] || 0) ^ 0xcb; // Coinbase prefix pattern
    }
    return sig;
  }
