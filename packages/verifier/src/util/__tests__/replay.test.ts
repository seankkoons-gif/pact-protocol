...
describe("v4 Transcript Replay", () => {
  describe("Genesis Hash (Round 0 previous_round_hash)", () => {
    it("should compute genesis hash matching SDK format: intent_id:created_at_ms", async () => {
      ...
      const hash = await computeInitialHash(intentId, createdAtMs);
    ...
    it("should match SUCCESS-001 fixture round 0 previous_round_hash", async () => {
      const transcript = loadFixture("fixtures/success/SUCCESS-001-simple.json");
      const round0 = transcript.rounds[0];
      
      const computedGenesis = await computeInitialHash(transcript.intent_id, transcript.created_at_ms);
  ...
  describe("Fixture Round Hashes", () => {
    ...
    it("should chain previous_round_hash correctly through all rounds", async () => {
      const transcript = loadFixture("fixtures/success/SUCCESS-001-simple.json");
      
      // Round 0: previous = genesis
      const genesis = await computeInitialHash(transcript.intent_id, transcript.created_at_ms);
